<div class="px-4 py-3">
	...
	<div class="flex flex-col lg:flex-row items-center mb-3 lg:my-6 items-center">
		@if ($mvp)
			<img src="{{ $mvp->player->getImg() }}" alt="{{ $mvp->player->name }}" class="rounded-full border border-gray-200 dark:border-gray-650 object-cover w-20 h-20 md:w-28 md:h-28 lg:w-36 lg:h-36 mt-3 lg:mt-0">
		@endif
		<div class="flex flex-col lg:ml-3 my-2 lg:my-0 text-center lg:text-left">
			<div class="flex flex-col lg:flex-row items-center">
				<div class="flex flex-col lg:mr-3">
					<span class="text-xl font-bold lg:pl-2.5">{{ $mvp ? $mvp->player->name : '-' }}</span>
					<span class="text-sm uppercase lg:pl-2.5">{{ $mvp ? $mvp->player->getPosition() : '' }}</span>
				</div>
				@if ($mvp)
					<img src="{{ $mvp->seasonTeam->team->getImg() }}" class="w-12 h-12 mx-auto">
				@endif
			</div>
