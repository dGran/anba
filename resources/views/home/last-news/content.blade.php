        ...
        @foreach ($posts as $post)
            <div class="flex justify-between p-4 border-b border-gray-200 dark:border-gray-650" wire:loading.class="opacity-50">
                ...
                @elseif ($post->type == "lesiones" && $post->team)
                    <figure class="h-20 w-20 md:h-24 md:w-24 rounded-md shadow-md border border-gray-200 dark:border-gray-650 relative" style="{{ $post->team ? 'background-color: ' . $post->team->color : '' }}">
                        <img src="{{ $post->getImg() }}" alt="{{ $post->getName() }}" class="h-full w-auto rounded-md object-cover mx-auto">
                        <img src="{{ $post->team->getImg() }}" alt="{{ $post->team->getName() }}" class="absolute bottom-0 left-0 m-1 w-6 h-6 md:w-7 md:h-7 object-cover rounded bg-white border border-gray-300">
                    </figure>
                @else
