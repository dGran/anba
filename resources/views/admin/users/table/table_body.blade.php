<tbody>
	@foreach ($regs as $reg)
		<tr class="{{ $regsSelected->find($reg->id) ? 'selected' : '' }} {{ $reg->email_verified_at ?: 'text-gray-500' }}" wire:click.stop="checkSelected({{ $reg->id }})">
			<td clas="check" style="min-width: 200px; padding: 0">
				<div class="d-flex align-items-center" style="height: inherit; padding: .5em 1em; }}">
				    ...
				    @if ($showTableImages)
						<img class="image rounded-circle non-selectable" src="{{ $reg->getImg() }}" alt="{{ $reg->name }}" style="width: 40px; {{ !$reg->email_verified_at ? '-webkit-filter: grayscale(100%); filter: grayscale(100%);' : '' }}">
				    @endif
					<div class="pl-2">
						<div class="name d-flex align-items-center" wire:click.stop="view({{ $reg->id }})">
							<span>{{ $reg->name }}</span>
						</div>
						<p class="{{ !$reg->email_verified_at ?: 'text-muted' }} text-xxs m-0">
			...
			<td class="{{ $colEmail ?: 'd-none' }}" style="width: 300px;">
				{{ $reg->email }}
			</td>
		</tr>
	@endforeach
