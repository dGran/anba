    ...
    >
        ...
        <div class="preview w-100 rounded position-relative" style="border: 1px solid #ced4da; position: relative">
            ...
            <figure class="pt-4 pb-3 m-0">
                @if ($reg_img)
                    <img src="{{ $img ? $img->temporaryUrl() : $reg_img_formated }}" class="object-cover rounded d-block m-auto" alt="Imagen de usuario" style="width: auto; height: 120px; cursor: pointer" onclick="$('#inputFile').trigger('click');">
                @else
                    <img src="{{ $img ? $img->temporaryUrl() : 'http://placehold.it/120x120' }}" class="object-cover rounded d-block m-auto" alt="Imagen de usuario" style="width: auto; height: 120px; cursor: pointer" onclick="$('#inputFile').trigger('click');">
                @endif
            </figure>
...
<div class="form-row">
    ...
    <div class="form-group col-md-6 @error('email') error @enderror">
        ...
        <input type="text" class="form-control text-sm" placeholder="E-Mail..." wire:model="email">
        @error('email')
            <p class="text-xs pt-1 m-0">{{ $message }}</p>
        @enderror
    </div>
...
@if (!$editMode)
    <div class="form-row">
        <div class="form-group col-md-6 @error('password') error @enderror">
            <label class="text-sm text-uppercase tracking-wide">Contraseña</label>
            <input type="password" class="form-control text-sm" placeholder="Contraseña..." wire:model="password">
            @error('password')
                <p class="text-xs pt-1 m-0">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="text-xs mr-auto">
            <div class="pretty p-svg p-curve m-0 p-jelly p-has-focus">
                ...
                <div class="state p-primary d-flex align-items-center">
                    ...
                    <label class="text-xs text-uppercase tracking-widest ml-1" style="">Cuenta verificada</label>
                </div>
            </div>
        </div>
    </div>
...
@if ($editMode)
    <p class="font-bold text-danger text-center">Por motivos de seguridad la contraseña no puede ser editada.</p>
@endif