<tbody>
	@foreach ($regs as $reg)
		<tr class="{{ $regsSelected->find($reg->id) ? 'selected' : '' }} {{ $reg->current ?: 'text-gray-500' }}" wire:click.stop="checkSelected({{ $reg->id }})">
			<td clas="check" style="min-width: 160px; padding: 0">
				<div class="d-flex align-items-center" style="height: inherit; padding: .5em 1em;">
					...
					<div class="pl-2">
						...
						<p class="{{ !$reg->current ?: 'text-muted' }} text-xxs m-0">
			...
			<td style="width: 128px; min-width: 128px; min-width: 128px">
                ...
                <a wire:click.stop href="{{ route('admin.playoffs', $reg) }}" class="ml-2 btn btn-primary text-xs text-uppercase tracking-widest text-white">
                    Playoffs
                </a>
			</td>
		</tr>
