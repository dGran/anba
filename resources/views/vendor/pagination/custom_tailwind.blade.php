        ...
        <ul class="pagination">
            ...
            @foreach ($elements as $element)
                ...
                @if (is_string($element))
                    <li class="hidden md:block page-item separator disabled" aria-disabled="true"><span class="page-link">{{ $element }}</span></li>
                @endif
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        ...
                        @else
                            <li class="hidden md:block page-item"><a class="page-link" tabindex="0" wire:click="gotoPage({{ $page }})">{{ $page }}</a></li>
                        @endif
                    @endforeach
                @endif
            @endforeach
